...
def movePlayer():
    ...
    # Pause
    # Update the screen
    turtle.ontimer(movePlayer, t=30)
...
def movePipes():
    ...
    for pipe_pair in pipes:
        ...
        # kiểm tra bird có va chạm 2 thành ống bottom và top
        if (player.ycor() + 10 > pipe_top.ycor() - 180) or (player.ycor() - 10 < pipe_bottom.ycor() + 180):
            
            # Nếu không va chạm, ta sẽ kiểm tra xem bird có bị kẹt trong 1 ống nước hay không
            # Nếu có Game over
        ...
        # Mỗi khi bird vượt qua ống nước, + 1 điểm
        if pipe_top.xcor() + 30 < player.xcor() - 10:
            player.score += pipe_top.value
            pipe_top.value = 0


    turtle.ontimer(movePipes, t=20)
